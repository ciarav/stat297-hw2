...
for(i in 1:length(sp500names)){
  stock1 <- sp500names[i]

getSymbols(stock1, from = three_years_ago, to = today)


# Compute returns
Ra <- na.omit(ClCl(get(stock1)))


temp_name <- paste(stock1,".txt")

file_name <- gsub(" ", "", temp_name)
print(file_name)
write.table(Ra, file=file_name, row.names=FALSE, col.names=FALSE)

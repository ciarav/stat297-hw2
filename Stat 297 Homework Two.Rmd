...
p <- ggplot()
states <- subset(all_states, region %in% c( "illinois", "indiana", "iowa","kansas", 'maryland', "michigan", "minnesota","missouri",'nebraska','new jersey', "pennsylvania", "ohio", "south dakota", "wisconsin",'west virginia') )

p <- ggplot()
p <- p + geom_polygon( data=states, aes(x=long, y=lat, group = group),colour="black", fill="grey" )
p

p<- p + geom_point(data = uni_coord, aes(V1, V2 ,col = V4), size= 3) + xlab("Longitude") + ylab("Latitude")
p

#central_map <- get_map(location = 'Indiana', zoom = 5, maptype = "toner")
...
p
animation_opts(p, frame = 500, transition = 500, easing = "linear",
